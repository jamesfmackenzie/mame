...
#
# DIP switch setup (Tab → DIP Switches):
...
#
# Usage:
#   ./ridgerac3m-3screen.sh          normal run
#   ./ridgerac3m-3screen.sh debug    right screen gets -debug
...
DIP switch reminder (Tab → DIP Switches):
  left window   → SW2:1=ON,  SW2:2=OFF  (Left, PCB 1)
  center window → SW2:1=OFF, SW2:2=ON   (Center/Main, PCB 2)
  right window  → SW2:1=OFF, SW2:2=OFF  (Right, PCB 3)
